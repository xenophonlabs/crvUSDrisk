...
class PriceGenerator:
    ...
    def gen_single_jump_gbm(self,S0, mu, sigma, dt, T,jump_list,jump_direction,recovery_perc,recovery_speed):
        n_steps = int(T / dt)
        # List of ordered pairs (jump_size, probability)        
        jump_list = sorted(jump_list, key=lambda x: x[1])  # Sort by jump probability ascending                       
    ...
    def gen_cor_matrix(self,n_assets,sparse_cor):
        ...
        for i in range(n_assets):
            for j in range(n_assets):
                pair = sorted([i,j])
                if i==j: cor_matrix[i][j]=1.0
                else: cor_matrix[i][j]=sparse_cor[pair[0]][pair[1]]    
        return(cor_matrix)    

    def gen_cor_jump_gbm(self,n_assets,T,dt,mu,sigma,S0,cor_matrix,jump_list,jump_direction,recovery_perc,recovery_speed):
        n_steps = int(T / dt)
    ...
    def plot_gbms(self,S,n_assets):
        ...
        if n_assets>1:
            for i in range(n_assets): plt.plot(S[:, i], label=f'Asset {i+1}')
        else:
            plt.plot(S, label=f'Asset')
